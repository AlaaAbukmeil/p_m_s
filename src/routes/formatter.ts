...
formatterRouter.post("/ib-excel", verifyToken, uploadToBucket.any(), async (req: Request | any, res: Response, next: NextFunction) => {
  try {
    ...
    let trades = await getAllTrades(beforeMonth, now, "portfolio_main", "ib");
...
formatterRouter.post("/mufg-excel-cds", verifyToken, uploadToBucket.any(), async (req: Request | any, res: Response, next: NextFunction) => {
  ...
  let trades = await getAllTrades(data.timestamp_start, data.timestamp_end, "portfolio_main", "cds_gs");
...
formatterRouter.post("/centralized-blotter", verifyToken, uploadToBucket.any(), async (req: Request | any, res: Response, next: NextFunction) => {
  try {
    ...
    let cdsTrades: [CentralizedTrade[], number] | any = await await getAllTrades(start, end, "portfolio_main", "cds_gs");
    let canceledTrades = await getAllTrades(start, end, "portfolio_main", "canceled_vcons");
    ...
    let ibTrades = await getAllTrades(start, end, "portfolio_main", "ib");
    let emsxTrades = await getAllTrades(start, end, "portfolio_main", "emsx");
...
formatterRouter.post("/emsx-excel", verifyToken, uploadToBucket.any(), async (req: Request | any, res: Response, next: NextFunction) => {
  try {
    ...
    let now = new Date().getTime() + 5 * 24 * 60 * 60 * 1000;
    let trades = await getAllTrades(beforeMonth, now,"portfolio_main",  "emsx");
