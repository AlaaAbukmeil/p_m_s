...
import { formatDateReadable, getDate, monthlyRlzdDate, uri } from "./common";
import { getEarliestCollectionName, getSecurityInPortfolio, getPortfolio, insertTradesInPortfolio, tradesTriadaIds, returnPositionProgress, updateExisitingPosition, getHistoricalPortfolio, getBBTicker } from "./reports";
...
export async function editTrade(editedTrade: any, tradeType: any) {
  try {
    ...
    if (tradeInfo) {
      let centralizedBlotKeys: any = ["B/S", "Issue", "BB Ticker", "Location", "Trade Date", "Trade Time", "Settle Date", "Price", "Notional Amount", "Settlement Amount", "Principal", "Counter Party", "Triada Trade Id", "Seq No", "ISIN", "Cuisp", "Currency", "Yield", "Accrued Interest", "Original Face", "Comm/Fee", "Trade Type", "Edit Note"];
      let changes = 0;
      let changesText = [];
...
export async function recalculateRlzd(data: any, date: any) {
  try {
    ...
    let portfolio = await getHistoricalPortfolio(lastMonthLastCollectionName[0]);
    let upToDatePortfolio = await getPortfolio();
