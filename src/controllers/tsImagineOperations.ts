...
export async function checkIfSecurityExist(path: string) {

    const data: any = await readIsinExcel(path)
    // return data[0]
    if (data.error) {
        return data
    } else {
        ...
        let action = await axios({
            method: 'post', // or 'post', 'put', etc., depending on what you need
            url: url,
            data: isin,
            auth: {
...
async function readIsinExcel(path: string) {
...
export async function sendIsinComplaint(data: any) {
