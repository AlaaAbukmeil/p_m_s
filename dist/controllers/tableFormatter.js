...
function formatGeneralTable(portfolio, date, fund, dates, conditions = null) {
    ...
    for (let index = 0; index < portfolio.length; index++) {
        ...
        position["DV01 Dollar Value Impact Utilization % of Nav"] = Math.round(((0, common_1.parsePercentage)(position["DV01 Dollar Value Impact % of Nav"]) / (0, common_1.parsePercentage)(position["DV01 Dollar Value Impact Limit % of Nav"])) * 10000) / 100 + " %";
        position["DV01 Dollar Value Impact Test"] = Math.abs((0, common_1.parsePercentage)(position["DV01 Dollar Value Impact Utilization % of Nav"])) < 100 ? "Pass" : "Fail";
        ...
        position["Value (BC) Limit % of Nav"] = Math.abs((0, common_1.parsePercentage)(position["Value (BC) % of Nav"])) > 10 ? 15 : 10 + " %";
        position["Value (BC) Utilization % of Nav"] = Math.round(((0, common_1.parsePercentage)(position["Value (BC) % of Nav"]) / (0, common_1.parsePercentage)(position["Value (BC) Limit % of Nav"])) * 10000) / 100 + " %";
        position["Value (BC) Test"] = Math.abs((0, common_1.parsePercentage)(position["Value (BC) Utilization % of Nav"])) < 100 ? "Pass" : "Fail";
...
function formatFrontEndSummaryTable(portfolio, date, fund, dates, sort, sign, conditions = null) {
    let formattedPortfolio = formatGeneralTable(portfolio, date, fund, dates, conditions);
    let formatted = [];
...
function sumTable(table, data, view, param, subtotal = false, subtotalParam = "") {
    try {
        ...
        let dv01DollarValueOfNav = (0, common_1.parsePercentage)(data["DV01 Dollar Value Impact % of Nav"]);
        let dv01DollarValueLimitOfNav = (0, common_1.parsePercentage)(data["DV01 Dollar Value Impact Limit % of Nav"]);
        let dv01DollarValueLimitUtilization = (0, common_1.parsePercentage)(data["DV01 Dollar Value Impact Utilization % of Nav"]);
        let dv01DollarValueImpactTest = data["DV01 Dollar Value Impact Test"];
        let valueUSDOfNav = (0, common_1.parsePercentage)(data["Value (BC) % of Nav"]);
        //gmv only for front office
        let valueUSDOfGmv = 0;
        if (view == "frontOffice") {
            valueUSDOfGmv = (0, common_1.parsePercentage)(data["Value (BC) % of GMV"]) || 0;
        }
        let valueUSDLimitOfNav = (0, common_1.parsePercentage)(data["Value (BC) Limit % of Nav"]);
        let valueUSDUtilizationOfNav = (0, common_1.parsePercentage)(data["Value (BC) Utilization % of Nav"]);
        let valueUSDOfNavTest = data["Value (BC) Test"];
        let capitalGains = parseFloat(data["Capital Gain/ Loss since Inception (Live Position)"]);
        let capitalGainsPercentage = (0, common_1.parsePercentage)(data["% of Capital Gain/ Loss since Inception (Live Position)"]);
        ...
        let totalCaptialGainsPercentage = (0, common_1.parsePercentage)(data["% of Total Gain/ Loss since Inception (Live Position)"]);
...
function assignColorAndSortParamsBasedOnAssetClass(pairHedgeNotional, pairIGNotional, pairHedgeDV01Sum, pairIGDV01Sum, globalHedgeNotional, singleIGNotional, globalHedgeDV01Sum, singleIGDV01Sum, hedgeCurrencyNotional, HYNotional, HYDV01Sum, cdsNotional, countryNAVPercentage, sectorNAVPercentage, strategyNAVPercentage, longShortDV01Sum, durationSummary, groupedByLocation, view, ustTable, igTable, hyTable, currTable, issuerTable, ustTableByCoupon, issuerNAVPercentage, rvPairTable, tickerTable) {
    for (let locationCode in groupedByLocation) {
        ...
        let groupDayPl = 0, groupMonthlyPl = 0, groupDV01Sum = 0, groupUSDMarketValue = 0, groupDuration = 0, groupRating = -99, groupOAS = 0, groupOASWChange = 0, groupZSpread = 0, groupNotional = 0, groupSpreadTZ;
