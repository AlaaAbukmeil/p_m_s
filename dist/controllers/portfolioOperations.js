...
exports.insertPricesUpdatesInPortfolio = exports.updatePricesPortfolio = exports.insertTradesInPortfolio = exports.updatePositionPortfolio = exports.uploadPortfolioFromImagine = exports.getBBTicker = exports.insertTrade = exports.getDailyEarnedInterestRlzPtf = exports.getTrades = exports.getPortfolio = exports.getPortfolioWithAnalytics = exports.getHistoricalPortfolio = exports.getHistoricalPortfolioWithAnalytics = void 0;
...
async function getPortfolioWithAnalytics() {
    try {
        const database = client.db("portfolios");
...
exports.getPortfolioWithAnalytics = getPortfolioWithAnalytics;
...
function getSecurityInPortfolio(portfolio, identifier, isinOrIssue) {
    let document = 404;
...
async function insertTrade(trades) {
    const database = client.db("trades");
    const reportCollection = database.collection("vcons");
    ...
    if (data.error) {
        ...
        try {
            ...
            for (let index = 0; index < data.length; index++) {
                ...
                let object = {};
                let securityInPortfolio = 404; //getSecurityInPortfolio(portfolio, identifier, issueOrBBTicker)//404// 
                ...
                let currentQuantity = parseInt(row["Quantity"]); //
                ...
                if (row["Status"] == "Accepted" && !(tradeExistsAlready) && (identifier)) {
                    if (!positions[identifier]) {
                        ...
                        object["Average Cost"] = securityInPortfolio !== 404 ? (0, portfolioFunctions_1.getAverageCost)(currentQuantity, previousQuantity, currentPrice, previousAverageCost) : (currentPrice) ? currentPrice : 0;
                        object["Coupon Rate"] = bondCouponMaturity[0];
                        object["Maturity"] = bondCouponMaturity[1] !== "Not Applicable" ? (0, common_1.formatDate)(bondCouponMaturity[1]) : bondCouponMaturity[1];
...
async function updatePositionPortfolio(path) {
    let data = await (0, portfolioFunctions_1.readVconEBlot)(path);
    if (data.error) {
        return data;
    }
    else {
        try {
            ...
            for (let index = 0; index < data.length; index++) {
                ...
                let object = {};
                let securityInPortfolio = getSecurityInPortfolio(portfolio, identifier, isinOrIssue);
                let previousQuantity = securityInPortfolio["Quantity"];
                let previousAverageCost = (securityInPortfolio["Average Cost"]) ? (securityInPortfolio["Average Cost"]) : 0;
                let tradeType = row["Buy/Sell"];
                let operation = tradeType == "B" ? 1 : -1;
                let currentPrice = row["Price"] / 100;
                let currentQuantity = parseInt(row["Quantity"].replace(/,/g, '')) * operation;
                ...
                if (row["Status"] == "Accepted" && !(tradeExistsAlready) && identifier !== "") {
                    if (!positions[identifier]) {
                        ...
                        object["Average Cost"] = securityInPortfolio !== 404 ? (0, portfolioFunctions_1.getAverageCost)((currentQuantity + positions[identifier]["Quantity"]), previousQuantity, currentPrice, previousAverageCost) : (0, portfolioFunctions_1.getAverageCost)(currentQuantity + samePositionQuantity, previousQuantity, currentPrice, previousAverageCost);
            ...
            try {
                let logs = `date: ${(0, common_1.getDate)() + " " + (0, common_1.getTime)()} e-blot-link: ${path}} \n\n`;
...
async function updatePricesPortfolio(path) {
    try {
        ...
        if (data.error) {
            ...
            for (let index = 0; index < data.length; index++) {
                let row = data[index];
                let object = getSecurityInPortfolio(portfolio, row["BB Ticker"].replace("Corps", "").replace("Govt", ""), 1);
                if (object == 404) {
                    break;
                }
                delete object["_id"];
...
function calculateMonthlyURlzd(portfolio) {
    for (let index = 0; index < portfolio.length; index++) {
